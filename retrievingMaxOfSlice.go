...
type MaxInterface interface{
    ...
    Get(i int) interface{}
    //Bigger returns whether the element at index i id bigger than the j one
    Bigger(i, j int) bool
    }
...
//Get implementation for three types
...
func Max(data MaxInterface) (ok bool, max interface{}) {
    if data.Len() == 0{
        return false,nil // no elements in the collection
        }
    if data.Len() == 1 { // Only one elment, return it alongside with true
